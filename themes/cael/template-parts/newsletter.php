...
<form action="code mailchimp spécifique" method="post" id="cael-embedded-subscribe-form" name="cael-embedded-subscribe-form" class="validate" target="_blank" novalidate="">
    <div id="cael-embed-signup" class="grid-x align-center">
			<label for="mce-EMAIL"><h3><?php  echo esc_html( $titre ); ?></h3><br><?php  echo esc_html( $soustitre ); ?></label>
			<input value="" name="EMAIL" class="email" id="mce-EMAIL" placeholder="<?php  echo esc_html( $saisie ); ?>" required="" data-cip-id="à modifier" type="email" onclick="jQuery('#newsletter-info').show()">
			...
			<input value="<?php  echo esc_html( $bouton ); ?>" name="subscribe" id="cael-embedded-subscribe" class="button" onclick="if(!this.form.consent.checked){jQuery('#alerte').show();return false}" type="submit">
